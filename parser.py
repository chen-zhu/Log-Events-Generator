...
class parser:
    ...
    def parsingFile(self, file_path):
        print("Parsing rule file... <File Path:" + file_path + ">")
    ...
    def tokenize(self, parsed_result):
        ...
        for header in self.raw_mapping_header:
            event_name = header[0]
            self.header_all_columns[table_name] = []
            ...
            for col in header[1]:
                if isinstance(col, (float, int, str)):
                    ...
                    self.header_all_columns[table_name].append(col)
                else:
                    self.header_condition_columns[event_name].append(col)
                    self.header_all_columns[table_name].append(col[0])
